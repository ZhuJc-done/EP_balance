...
def _make_case(seed: int, R_nodes=2, R_gpus=2, E=8, skew=2.0):
    ...
    # Skewed integer load matrix Ω[R, E].
...
def _units_for_rank(omega_row: torch.Tensor, seed: int) -> torch.Tensor:
    """Build units containing exactly ``omega_row[e]`` entries for expert ``e``."""
    g = torch.Generator().manual_seed(seed)
    units = torch.repeat_interleave(torch.arange(omega_row.numel()), omega_row)
...
def test_assign_physical_matches_reference_dst():
    for seed in range(20):
        ...
        R = topo.num_ranks
        for r in range(R):
            unit_expert = _units_for_rank(loads.omega[r], seed * 100 + r)
...
def test_phys_id_decodes_to_hosting_expert():
    ...
    for r in range(R):
        unit_expert = _units_for_rank(loads.omega[r], 500 + r)
...
def test_no_host_sync_ops_on_cuda():
    ...
    r = 0
    unit_expert = _units_for_rank(loads.omega[r], 11).to(dev)
